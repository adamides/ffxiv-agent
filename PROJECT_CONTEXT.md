...
- **Mit-sheet design conventions — shared, sanitized.** The generic, reusable model for a comp-agnostic mit-plan sheet (role columns, Party Mit vs Extras, oGCD-lean barriers, school tag per row, no %-mit on failure/tower rows, busters on tank-pairing tabs) was lifted into [docs/mit-sheet-conventions.md](docs/mit-sheet-conventions.md). The owner-specific/private details it was extracted from (real comp, prog state, private sheet/doc IDs, the SA email/key path) stay out of the repo — they live only in the owner's `~/.claude` build-agent memories, which are **not** shared.
